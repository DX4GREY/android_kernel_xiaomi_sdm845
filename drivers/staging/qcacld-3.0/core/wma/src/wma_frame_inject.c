...
struct wma_injection_queue_ctx {
	...
	qdf_delayed_work_t delayed_work;
	qdf_delayed_work_t reaper_work; /* periodic stale-nbuf reaper */
			    ...
			    uint32_t chanfreq)
{
	 ...
	 * to WAL-TX path in the firmware mgmt TX handler without beacons.
	 */
	...
	/* 2.4 GHz -> MODE_11G, 5 GHz -> MODE_11A */
	vstart.chan_mode         = (chanfreq < 4000) ?
...
static void wma_process_injection_queue_work(void *arg)
{
	...
	if (queue_has_frames) {
		...
		if (backpressure_delay > 0) {
			qdf_sched_delayed_work(&ctx->delayed_work,
					       backpressure_delay);
		} else {
...
static void wma_injection_reaper_work_cb(void *context)
{
	...
	if (ctx->is_initialized)
		qdf_sched_delayed_work(&ctx->reaper_work,
...
QDF_STATUS wma_init_injection_queue(tp_wma_handle wma_handle)
{
	...
	qdf_create_delayed_work(&ctx->delayed_work,
				wma_process_injection_queue_delayed_work, NULL);

	/* Initialize nbuf-leak reaper timer */
	qdf_create_delayed_work(&ctx->reaper_work,
					  ...
					  uint8_t vdev_id)
{
	...
	if (monitor_vdev && is_probe_req && is_bcast_da && req->frame_len >= 24) {
		uint8_t *vdev_mac = wma_handle->interfaces[vdev_id].addr;
	...
	if (!wmi_tx_ok) {
		...
		ret = cdp_mgmt_send_ext(soc,
					wma_handle->interfaces[
						mgmt_params.vdev_id].handle,
					wmi_buf, mgmt_params.tx_type,
