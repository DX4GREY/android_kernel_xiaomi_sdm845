...
void hdd_process_injection_queue_work(void *arg)
{
	...
	while (true) {
		...
		if (injection_ctx->wma_handle) {
			uint8_t tx_vdev_id = injection_ctx->adapter->session_id;
			uint8_t mon_adapter_vdev_id = 0xff;
			bool mon_adapter_open = false;
			bool monitor_mode_active = injection_ctx->is_monitor_mode;
			...
			if (monitor_mode_active) {
				...
				if (mon_adapter) {
					mon_adapter_vdev_id =
						mon_adapter->session_id;
					mon_adapter_open =
						test_bit(DEVICE_IFACE_OPENED,
							 &mon_adapter->event_flags);
				}

				if (mon_adapter_open && mon_adapter_vdev_id != 0xff)
					tx_vdev_id = mon_adapter_vdev_id;
			}

			if (!queue_tx_path_logged) {
				hdd_inject_info("injection queue tx path: adapter_vdev=%u monitor_mode=%u monitor_adapter_vdev=%u monitor_adapter_open=%u tx_vdev=%u",
						injection_ctx->adapter->session_id,
